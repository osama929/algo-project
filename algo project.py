...
class SortingVisualizer:
    def _init_(self, root):
        ...
        tk.Label(input_frame, text="Enter numbers (comma separated):", font=("Arial", 11), bg="#f0f0f0").grid(row=0, column=0, padx=5)
        self.entry = tk.Entry(input_frame, width=40, font=("Arial", 11))
        self.entry.grid(row=0, column=1, padx=5)

        tk.Button(input_frame, text="Generate Bars", command=self.draw_data, font=("Arial", 10), bg="#4CAF50", fg="white", width=15).grid(row=0, column=2, padx=5)

        # Button Frame
        btn_frame = tk.Frame(root, bg="#f0f0f0")
        btn_frame.pack(pady=10)

        tk.Button(btn_frame, text="Bubble Sort", command=self.bubble_sort, font=("Arial", 10), bg="#2196F3", fg="white", width=15).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Merge Sort", command=self.merge_sort_start, font=("Arial", 10), bg="#9C27B0", fg="white", width=15).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Quick Sort", command=self.quick_sort_start, font=("Arial", 10), bg="#FF5722", fg="white", width=15).pack(side=tk.LEFT, padx=10)
    ...
    def draw_bars(self, data, color_array=None):
        ...
        for i, height in enumerate(normalized_data):
            ...
            self.canvas.create_text(x0 + 2, y0, anchor=tk.SW, text=str(data[i]), font=("Arial", 9))
